  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const match = await api.post<{ id: string }>('/matches', {
        ...
        playerTeam: playerTeam.trim() || undefined,
      });
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div className="rounded-xl border border-white/[0.08] bg-ink2/60 backdrop-blur-xl p-6">
          ...
          <div className="space-y-4">
            ...
            {/* Player team */}
            <div>
              <label htmlFor="playerTeam" className="mb-1.5 block text-sm font-medium text-[#f8f8fc]/80">
                Time que você jogou <span className="text-[#f8f8fc]/35">(opcional)</span>
              </label>
              <input
                id="playerTeam"
                type="text"
                maxLength={100}
                placeholder="Ex: Real Madrid, ou &quot;camisa azul, ataca da esquerda no 1º tempo&quot;"
                value={playerTeam}
                onChange={(e) => setPlayerTeam(e.target.value)}
                className="w-full rounded-lg border border-white/10 bg-white/[0.06] px-3 py-2.5 text-sm text-[#f8f8fc] placeholder-white/30 focus:border-gold focus:outline-none focus:ring-1 focus:ring-gold/50"
              />
              <p className="mt-1.5 text-xs text-[#f8f8fc]/35">
                Ajuda a IA a identificar seu time no vídeo (e não o do adversário) antes de apontar os erros.
              </p>
            </div>
          </div>
        </div>
